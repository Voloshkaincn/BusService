...
var config = {
    ...
    baseSassPath: baseSrcPath + 'sass/base',
    blocksPath: baseSrcPath + 'blocks'
};
...
gulp.task('js', function () {
    gulp.src(['src/js/**/*.js', '!src/js/libs.js', 'src/blocks/**/*.js'])
        .pipe(concat('main.js'))
        .pipe(uglify())
        .pipe(rename({suffix: '.min'}))
        .pipe(gulp.dest('dev/js'))
...
gulp.task('styles', function () {
    return gulp.src(['src/sass/**/*.scss', '!src/sass/**/_*.scss', 'src/blocks/**/*.scss'])
        ...
        .pipe(sass({includePaths: [config.baseSassPath]}))
        .pipe(autoprefixer(['last 15 versions', '> 1%', 'ie 8', 'ie 7'], {cascade: true}))
        .pipe(cssnano())
        .pipe(rename({suffix: '.min'}))
...
gulp.task('watch', ['browser-sync', 'libs-css', 'styles', 'libs-js', 'js', 'include', 'img', 'fonts'], function () {
    gulp.watch(['src/sass/**/*.scss', 'src/blocks/**/*.scss'], ['styles']);
